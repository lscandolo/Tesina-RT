...
int32_t 
SecondaryRayGenerator::generate_disc(Scene& scene, RayBundle& ray_in, size_t rays_in,
                                     HitBundle& hits, RayBundle& ray_out, size_t* rays_out)
{
	...
	if (m_timing)
        ...
        DeviceMemory& count_mem = device.memory(count_id);
        if (sizeof(cl_int)*(2*rays_in+2) > count_mem.size()) {
                // *2 because we have one int per reflect ray and one int per refract ray
                if (count_mem.resize(sizeof(cl_int)*(2*rays_in+2))) 
                        return -1;
        }
        ...
        if (marker.set_arg(0, hits.mem()) || 
            ...
            marker.set_arg(5, sizeof(cl_int), &rays_in)) {
                return -1;
        }
        ...
        if (gpu_scan_uint(device, count_id, 2*rays_in+1, count_id)){
                return -1;
        }
        ...
        *rays_out = new_ray_count;
        ...
        if (*rays_out) {
                if (generator.set_arg(0, hits.mem()) || 
                    ...
                    generator.set_arg(6, sizeof(cl_int),&rays_in) ||
