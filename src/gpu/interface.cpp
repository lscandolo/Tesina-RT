        ...
        invalid_memory(invalid_clinfo), invalid_function(invalid_clinfo)
{
        ...
        memory_objects[0].resize(PREALLOC_SIZE,DeviceMemory(m_clinfo));

        function_map.resize(1);
        function_map[0].set();

        function_objects.resize(1);
        function_objects[0].resize(PREALLOC_SIZE,DeviceFunction(m_clinfo));
...
DeviceInterface::new_memory()
{
        ...
        for (size_t i = 0; i < memory_map.size(); ++i) {
                ...
                for (size_t j = 0; j < memory_map[i].size(); j++)
                        if (memory_map[i][j]) {
                                memory_map[i].flip(j);
                                memory_objects[i][j] = DeviceMemory(m_clinfo);
        ...
        memory_objects.push_back(
                std::vector<DeviceMemory>(PREALLOC_SIZE,DeviceMemory(m_clinfo)));
        
        memory_map[i].set();
        memory_map[i][0].flip();

        memory_objects[i][0] = DeviceMemory(m_clinfo);
...
DeviceInterface::new_function()
{
        ...
        for (size_t i = 0; i < function_map.size(); ++i) {
                ...
                for (size_t j = 0; j < function_map[i].size(); j++)
                        if (function_map[i][j]) {
                                ...
                                function_objects[i][j] = DeviceFunction(m_clinfo);
                                return (i*PREALLOC_SIZE + j);
                        }
        }
        ...
        function_objects.push_back(
                std::vector<DeviceFunction>(PREALLOC_SIZE,DeviceFunction(m_clinfo)));
        
        function_map[i].set();
        function_map[i][0].flip();

        function_objects[i][0] = DeviceFunction(m_clinfo);
        return i * PREALLOC_SIZE;
...
{
        return id >= 0 && 
                id < memory_map.size()*PREALLOC_SIZE && 
...
{
        return id >= 0 && 
                id < function_map.size()*PREALLOC_SIZE && 
...
int32_t 
DeviceInterface::enqueue_barrier(){
        if (!good())
                return -1;
...
DeviceInterface::copy_memory(memory_id from, memory_id to, 
                             size_t bytes, size_t from_offset, size_t to_offset) 
{
        if (!good())
                return -1;
